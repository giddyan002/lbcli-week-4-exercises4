# Create a raw transaction and add this message in it: "btrust builder 2026"

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

transaction=""

# Extract txid
txid=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.txid')

# Convert string to hex safely using python
hex_msg=$(python3 -c 'print("btrust builder 2026".encode("utf-8").hex())')

# Create raw transaction with OP_RETURN data
bitcoin-cli -regtest createrawtransaction \
  "[{\"txid\":\"$txid\",\"vout\":0},{\"txid\":\"$txid\",\"vout\":1}]" \
  "{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\":0.20000000,\"data\":\"$hex_msg\"}"